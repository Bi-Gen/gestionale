...
type Soggetto = {
  ...
  tipo?: string[] // Array: ['cliente'], ['fornitore'], o ['cliente', 'fornitore']
}
...
export default function MovimentoFormDynamic({
  causali,
  ...
  // Determina il tipo di soggetto in base alla causale
  const getTipoSoggetto = (causaleCode: string): 'cliente' | 'fornitore' | 'tutti' => {
    const causaliClienti = ['VEN', 'RES'] // Vendita, Reso da cliente
    const causaliFornitori = ['ACQ', 'RESF'] // Acquisto, Reso a fornitore
  ...
  // Filtra soggetti quando cambia la causale
  useEffect(() => {
    ...
    // Reset soggetto e prodotti quando cambia causale
  ...
  // Carica prodotti filtrati quando si seleziona un soggetto
  useEffect(() => {
    if (!soggettoSelezionato || !causaleSelezionata) {
      setProdottiFiltrati(prodottiIniziali)
      return
    }
    ...
    // Solo per causali specifiche (vendita/reso da cliente, acquisto/reso a fornitore)
    if (tipoSoggetto === 'tutti') {
      setProdottiFiltrati(prodottiIniziali)
      return
    }

    const tipoOperazione = tipoSoggetto === 'cliente' ? 'vendita' : 'acquisto'

    setLoadingProdotti(true)
    fetch(`/api/magazzino/prodotti-soggetto?soggetto_id=${soggettoSelezionato}&tipo=${tipoOperazione}`)
      .then(res => res.json())
      .then(data => {
        if (data && data.length > 0) {
          setProdottiFiltrati(data)
        } else {
          // Se non ci sono prodotti specifici, mostra tutti
          setProdottiFiltrati(prodottiIniziali)
        }
      })
      .catch(err => {
  ...
  }, [soggettoSelezionato, causaleSelezionata, prodottiIniziali])
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
        ...
        {/* Soggetto (prima del prodotto per il filtro) */}
        <div>
        ...
        <div className="sm:col-span-2">
          <label htmlFor="prodotto_id" className="block text-sm font-medium text-gray-700">
            ...
            {soggettoSelezionato && prodottiFiltrati.length < prodottiIniziali.length && (
              <span className="ml-2 text-xs text-blue-600">
                (Filtrato: prodotti {isCarico ? 'acquistati da' : 'venduti a'} questo soggetto)
              </span>
            )}
          </label>
          <select
            {...register('prodotto_id')}
            disabled={loadingProdotti}
